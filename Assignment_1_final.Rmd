...
We also calculated the rate per 1000 to easier interpret the results. We
...
We fitted a Poisson regression model using natural cubic splines (ns) via the splines package. To account for non-linear trends and interactions as requested, we specified the model as follows:

n ~ sex + ns(year, df=5) + ns(age_mid, df=6) + sex:ns(year, df=5) + sex:ns(age_mid, df=6) + ns(year, df=5):age_mid

We increased the degrees of freedom to 5 for year and 6 for age to capture the complex temporal and age-related patterns. The model resulted in a Residual Deviance of 3426.2 and an AIC of 12592, which is a substantial improvement over simpler parametric models and adequately approximates the trends observed in the GAM analysis.
